$(document).ready(function () {
  ...
  $(".insertar").click(function () {
    $('.titulo').html('Agregar Colores');   // Titulo del form   
    $("#formColores").trigger("reset"); // Reseteams el form
...
function listar(base, Toast) {
  ...
  Edit("#coloresAbm tbody", table); //Tomar datos para la Edicion
  deleteColores("#coloresAbm tbody", table); //Eliminar un slide
...
function cambioEstado(body, table, Toast) {
  ...
  $(body).on("click", "a.estado", function () {
    ...
    $.ajax({
      type: "POST",
      url: UrlBase + 'mipanel/colores/cambioEstado',
...
function Edit(body, table) {
  ...
  $(body).on("click", "a.editar", function () {
    ...
    $.ajax({
      type: "POST",
      url: UrlBase + 'mipanel/colores/getColoresJson',
      data: { Id: datos.id },
      dataType: "json",
      success: function (response) {
...
function deleteColores(body, table) {
  //Tomando desde el boton de edicion
  $(body).on("click", "a.eliminar", function () {
    ...
    swalButtons.fire({
      ...
      if (result.value) {
        ...
        $.ajax({
          type: "POST",
          url: UrlBase + 'mipanel/colores/deleteTalle',
          data: { Id: datos.id },
          dataType: "json",
          success: function (response) {
            if (response.success == true) {
