...
//Configuramos las alerts
  const Toast = Swal.mixin({
    ...
    $(".insertar").click(function() {
      $('.titulo').html('Agregar Impuesto');   // Titulo del form   
      $("#formImpuestos").trigger("reset"); // Reseteams el form
...
function listar(base,Toast) {
    ...
    var table = $("#impuestosAbm").DataTable({
        destroy: true,
        responsive: true,
        order: [[1, 'asc']],
        ajax: {
    ...
    Edit("#impuestosAbm tbody", table); //Tomar datos para la Edicion
    deleteImpuestos("#impuestosAbm tbody", table); //Eliminar
...
function submit(table,Toast) {
  $("#formImpuestos").submit(function(e) {
...
// Funcion para tomar los datos de la edicion y asignarlos a los imputs
 function Edit(body, table) {
    ...
    //Tomando desde el boton de edicion
      ...
      $.ajax({
        type: "POST",
        url: UrlBase+'mipanel/impuestos/getImpuestoJson',
        data: { Id: datos.impuesto_id},
        dataType: "json",
        success: function (response) {
            ...
            $("#porcentaje").val(response['data'].porcentaje)
        },//success
 ...
 function deleteImpuestos(body, table) { 
    //Tomando desde el boton de edicion
      ...
      swalButtons.fire({
        ...
        if (result.value) {
          ...
          $.ajax({
              type: "POST",
              url: UrlBase+'mipanel/impuestos/deleteImpuesto',
              data: { Id: datos.impuesto_id },
              dataType: "json",
              success: function (response) {
                if (response.success == true) {
