$(document).ready(function () {
  ...
  $(".insertar").click(function () {
    $('.titulo').html('Agregar Forma de Entrega');   // Titulo del form   
    $("#formEntregas").trigger("reset"); // Reseteams el form
    ...
    $('#entregas_id').val(0) // Reiniciamos el valor del file para validacion
    $('#detalle').val("") // Reiniciamos el valor del file para validacion
    $('#costo').val("") // Reiniciamos el valor del file para validacion
...
function listar(base, Toast) {
  ...
  var table = $("#entregasAbm").DataTable({
    destroy: true,
    responsive: true,
    order: [[1, 'asc']],
    ajax: {
      url: base + "mipanel/entregas/getEntregas",
      type: "post",
      dataType: "json",
    },
    rowCallback: function (row, data) {
      ...
      if (data.pidedirec == "1") {
        $('td:eq(4)', row).html("<div class='text-center'><a href='javascript:void(0);' class='pidedirec'><i class='fa  fa-toggle-on fa-2x text-green'></i></a></div>");
      } else {
        $('td:eq(4)', row).html("<div class='text-center'><a href='javascript:void(0);' class='pidedirec'><i class='fa  fa-toggle-off fa-2x text-green'></i></a></div>");
  ...
  Edit("#entregasAbm tbody", table); //Tomar datos para la Edicion
  deleteEntregas("#entregasAbm tbody", table); //Eliminar

  //Cambiar estado en datatable
  cambioEstado("#entregasAbm tbody", table, Toast);
  cambioPidedirec("#entregasAbm tbody", table, Toast);
...
function cambioEstado(body, table, Toast) {
  ...
  $(body).on("click", "a.estado", function () {
    ...
    $.ajax({
      type: "POST",
      url: UrlBase + 'mipanel/entregas/cambioEstado',
...
//Funcion para cambiar la obligatoriedad de la direccion 
//de entrega de los pedidos 
function cambioPidedirec(body, table, Toast) {
  // Mostrar un alert con el dato de la row
  $(body).on("click", "a.pidedirec", function () {
    var me = $(this);
    var datos = table.row($(this).parents("tr")).data();
    // Ejecutamos la accion y la enviamos al servidor 
    $.ajax({
      type: "POST",
      url: UrlBase + 'mipanel/entregas/cambioPidedirec',
      data: { Estado: datos.pidedirec, Id: datos.id },
      dataType: "json",
      success: function (response) {
        if (response.success == true) {
...
function Edit(body, table) {
  ...
  $(body).on("click", "a.editar", function () {
    ...
    $.ajax({
      type: "POST",
      url: UrlBase + 'mipanel/entregas/getEntrega',
      data: { Id: datos.id },
      dataType: "json",
      success: function (response) {
        //Asignamos los valores de cada input para que se muestren en el form
        $("#id").val(response['data'].id)
        $("#entregas_id").val(response['data'].entregas_id)
...
function deleteEntregas(body, table) {
  //Tomando desde el boton de edicion
  $(body).on("click", "a.eliminar", function () {
    ...
    swalButtons.fire({
      ...
      if (result.value) {
        ...
        $.ajax({
          type: "POST",
          url: UrlBase + 'mipanel/entregas/deleteEntrega',
          data: { Id: datos.id },
          dataType: "json",
          success: function (response) {
            if (response.success == true) {
