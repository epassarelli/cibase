...
class Talles  extends MX_Controller {
...
public function accion()
{
  ...
  if ($this->form_validation->run() == TRUE ) {
        ...
        $talle['id']        = $this->input->post('id');
        $talle['descripcion']  = $this->input->post('descripcion');
        
        // Pasar el switch
        switch ($opcion) {

            case 'insertar':
              $data['success'] = $this->setTalles($talle);
            break;

            case 'editar':
                $data['success'] = $this->updateTalles($talle);
            break;
        }
  ...
  public function setTalles($data){
    $talle['descripcion']   = $data['descripcion'];
    $this->Talles_model->setTalles($talle);
    return TRUE;
  }
  ...
  public function getTallesJson(){
    $parametros['id'] = $this->input->post('Id');
    $data['data'] = $this->Talles_model->getOneBy('talles','',$parametros,'');
    echo json_encode($data);
  }
  ...
  // Editar un sitios
  public function updateTalles($data){
    $talle['id']            = $data['id'];
    $talle['descripcion']   = $data['descripcion'];
    $this->Talles_model->update($data['id'], $talle);
    return TRUE;
  ...
  //Eliminando un registro de la tabla de talles
  public function deleteTalle()
  {
    $id = $this->input->post('Id');
    $this->Talles_model->deleteTalles($id);
  ...
  public function cambioEstado()
  {
   ...
   $estadoNuevo = $this->Talles_model->cambioEstado($id,$estado);
   echo json_encode(array('success' => TRUE,
                          'estado' => $estadoNuevo,
  ...
  function upload($archivo,$tipos)
    {
      ...
      $config['upload_path']          = 'assets/uploads/' . $sitio_id . '/talles/';
