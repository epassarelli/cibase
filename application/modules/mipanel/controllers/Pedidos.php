...
class Pedidos  extends MX_Controller
{
  ...
  public function pedidoValidation()
  {
    ...
    $this->form_validation->set_rules('apellido', 'Apellido', array('required', 'max_length[200]'), array(
      'required'   => '{field} es obligatorio',
      'max_length' => '{field} no puede exceder {param} caracteres -'
    ));

    $this->form_validation->set_rules('nombre', 'Nombre', array('required', 'max_length[200]'), array(
      'required'   => '{field} es obligatorio',
      'max_length' => '{field} no puede exceder {param} caracteres -'
    ));

    if ($this->input->post('domicilio_requerido') == '1') {
      $this->form_validation->set_rules('del_calle', 'Apellido', array('required', 'max_length[200]'), array(
        'required'   => '{field} es obligatorio',
        'max_length' => '{field} no puede exceder {param} caracteres -'
      ));

      $this->form_validation->set_rules('del_nro', 'Numero', array('required', 'numeric', 'greater_than[0]'), array(
        'required'   => '{field} es obligatorio',
